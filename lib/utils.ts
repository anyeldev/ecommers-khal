import { Option, Products, Review, SubProduct } from '@/types'
...
export const getRatingNote = (reviews: Review[]): number => {
  const ratingTotal = reviews.reduce(
    (accumulator: number, currentValue: Review): number => accumulator + currentValue?.rating,
    0
  )
...
export const getBestPriceWithDiscountFromProduct = (product: Products): number => {
  const data = product.subProducts.map((subProduct: SubProduct) => {
    return subProduct.options.map((options: Option) => {
      return options.discount ? discountPrice(options.price, options.discount) : options.price
    })
  })
...
export const getBestPriceWithoutDiscountFromProduct = (product: Products): number => {
  const data = product.subProducts.map((subProduct: SubProduct) => {
    return subProduct.options.map((options: Option) => {
